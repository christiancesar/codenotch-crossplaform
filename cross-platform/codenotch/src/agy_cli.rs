...
/// Discover installed official Antigravity CLI (Windows: `agy.exe` in `%LOCALAPPDATA%\agy\bin`
/// and `PATH`; Linux: extensionless `agy` in the XDG data home, `/usr/local/bin`, then `PATH`).
pub fn find_agy() -> Option<PathBuf> {
    if let Some(local) = dirs::data_local_dir() {
        // Windows installs agy.exe under %LOCALAPPDATA%; Linux uses an extensionless binary in the
        // XDG data home
        #[cfg(windows)]
        let candidate = local.join("agy").join("bin").join("agy.exe");
        #[cfg(not(windows))]
        let candidate = local.join("agy").join("bin").join("agy");
