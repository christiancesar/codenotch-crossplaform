...
pub fn install() -> Result<String, String> {
    ...
    // The hook binary has no extension outside Windows; keeping the name per-platform so the
    // installed command points at the real file on each OS
    #[cfg(windows)]
    const HOOK_BIN: &str = "codenotch-hook.exe";
    #[cfg(not(windows))]
    const HOOK_BIN: &str = "codenotch-hook";
